...
수식적으로 보면, general corpus $D = \{d_i\}_i$ where $d_i$ is document, labled task data $T = \{(x_i,y_i)\}_i$ where $x_i$ is text and $y_i \in Y$ is a label 에 대해, 목표는 coniditional probability for classification $f(x)=\hat{p}(y|x)$ 를 추정하는 model $f$ 를 학습하는 것이다. 
