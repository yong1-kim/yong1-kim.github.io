...
이 방법은 f ully fine-tuned weight 과 pre-trained weight 의 차이는 low-rank property 를 보인다는 점을 가정한다.
...
이 것은 $X ~ N(0, \sigma^2)$ 을 가정하여 $F(X) = \Pi(X/\sigma), where \Pi(\cdot) is cumulative distribution$ 가 된다.
